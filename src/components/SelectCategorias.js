...
const SelectCategorias = ({categoria,cambiarCategoria}) => {
     const [mostarSelect,cambiarMostrarSelect]=useState(false)
     const categorias = [
    ...
    const selectActive=(e)=>{
        cambiarCategoria(e.currentTarget.dataset.item)
    }
     return (
         <ContenedorSelect onClick={()=>cambiarMostrarSelect(!mostarSelect)}>
             <OpcionSeleccionada>{categoria}<IconoDown/></OpcionSeleccionada>
             {mostarSelect&&
             <Opciones>
                    {categorias.map(cat=><Opcion key={cat.id} data-item={cat.id} onClick={selectActive}><InconoCategoria id={cat.id}/> {cat.texto}</Opcion>)}
            </Opciones>                       
             }
         </ContenedorSelect>
     );
